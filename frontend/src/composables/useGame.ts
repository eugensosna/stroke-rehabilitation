...
import type { Ball, Brick, Paddle, GameState, Point2D } from '@/types/game'
...
export class GameArcade {
  ...
  public movePaddle(point: Point2D): void {
    ...
    if (oldX !== this.paddle.x) {
      console.log(`Paddle moved from ${oldX} to ${this.paddle.x}`)
    }
  }
