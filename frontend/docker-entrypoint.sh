...
for name in $(printenv | sed -n 's/^\(VITE_[A-Za-z0-9_]*\)=.*/\1/p'); do
  ...
  find "$JS_DIR" -name '*.js' -exec grep "${name}" {} \; -print -quit | grep -q . && echo "Replacing __${name}__ with ${escaped_value}" || echo "No placeholder for __${name}__ found in JS files"
  find "$JS_DIR" -name '*.js' -exec sed -i "s|__${name}__|${escaped_value}|g" {} +
done

echo "Starting server with the following environment variables:"
# printenv | grep '^VITE_'
